...
export class StarFieldComponent implements OnDestroy {
  ...
  // Movement inputs
  /**
   * Enable drift movement - stars slowly flow in a direction
   ...
   * Higher values = faster drift
   */
  public readonly driftSpeed = input<number>(1.0);

  /**
   * Drift direction as normalized vector [x, y, z]
   * Default: diagonal upward-left drift
   */
  public readonly driftDirection = input<[number, number, number]>([
    -0.1, 0.05, 0,
  ]);

  /**
   * Enable rotation - entire star field slowly rotates
   * Creates a cosmic spinning effect
   */
  public readonly enableRotation = input<boolean>(false);

  /**
   * Rotation speed in radians per second (default: 0.02)
   * Positive = counter-clockwise, negative = clockwise
   */
  public readonly rotationSpeed = input<number>(0.02);
  ...
  private movementCleanup: (() => void) | null = null;
  ...
  public constructor() {
    ...
    effect((onCleanup) => {
      ...
      const enableRotation = this.enableRotation();
  ...
  private rebuildStars(
    ...
    // Setup movement animation if enabled
    if (enableDrift || enableRotation) {
  ...
  /**
   * Setup movement animation (drift and/or rotation) using RenderLoopService
   *
   * Drift: Stars slowly flow in a direction, wrapping around when they exit
   * Rotation: Entire star field rotates around specified axis
   */
  private setupMovementAnimation(): void {
    // Clear any existing movement animation
    if (this.movementCleanup) {
      this.movementCleanup();
      this.movementCleanup = null;
    }

    // Store initial positions for drift wrapping
    const radius = this.radius();

    this.movementCleanup = this.renderLoop.registerUpdateCallback((delta) => {
      if (!this.object3d) return;
      ...
      if (this.enableRotation()) {
        ...
        switch (axis) {
          case 'x':
            this.object3d.rotation.x += delta * rotSpeed;
            break;
          case 'y':
            this.object3d.rotation.y += delta * rotSpeed;
            break;
          case 'z':
            this.object3d.rotation.z += delta * rotSpeed;
            break;
        }
      }
      ...
      if (this.enableDrift()) {
        ...
        const [dx, dy, dz] = this.driftDirection();

        // Move the entire star field
        this.object3d.position.x += dx * speed * delta;
        this.object3d.position.y += dy * speed * delta;
        this.object3d.position.z += dz * speed * delta;

        // Wrap position to keep stars in visible range
        // When position exceeds radius, wrap back
        const wrapThreshold = radius * 0.5;
        if (Math.abs(this.object3d.position.x) > wrapThreshold) {
          this.object3d.position.x =
            -Math.sign(this.object3d.position.x) * wrapThreshold * 0.9;
        }
        if (Math.abs(this.object3d.position.y) > wrapThreshold) {
          this.object3d.position.y =
            -Math.sign(this.object3d.position.y) * wrapThreshold * 0.9;
        }
        if (Math.abs(this.object3d.position.z) > wrapThreshold) {
          this.object3d.position.z =
            -Math.sign(this.object3d.position.z) * wrapThreshold * 0.9;
        }
      }
    });
