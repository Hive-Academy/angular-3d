...
Nx monorepo providing Angular libraries for 3D graphics (Three.js) and scroll animations (GSAP), with a showcase demo application.

## Architecture

**Pattern**: Nx Monorepo with Publishable Libraries

```
angular-3d-workspace/
├── apps/
│   ├── angular-3d-demo/          # Showcase application
│   └── angular-3d-demo-e2e/      # E2E tests (Playwright)
├── libs/
│   ├── angular-3d/               # Three.js wrapper library (@hive-academy/angular-3d)
│   └── angular-gsap/             # GSAP wrapper library (@hive-academy/angular-gsap)
├── task-tracking/                # Task management docs
...
- **Build System**: Nx 22.2.6
- **3D Graphics**: Three.js 0.182 with TypeScript types
...
# Building
npx nx build angular-3d-demo              # Build demo app
npx nx build @hive-academy/angular-3d     # Build angular-3d library
npx nx build @hive-academy/angular-gsap   # Build angular-gsap library
npx nx run-many -t build                  # Build all projects

# Testing
npx nx test angular-3d-demo               # Test demo app
npx nx test @hive-academy/angular-3d      # Test angular-3d library
npx nx run-many -t test                   # Test all projects
npx nx e2e angular-3d-demo-e2e            # Run E2E tests

# Linting & Type Checking
npx nx lint angular-3d-demo               # Lint demo app
npx nx run-many -t lint                   # Lint all projects
npx nx run-many -t typecheck              # Type check all projects

# Utilities
npx nx graph                              # Visualize project graph
npx nx show project angular-3d-demo       # Show project targets
...
- All 3D components extend or compose Three.js objects
- Use `SceneService` for accessing scene, camera, renderer
- Use `RenderLoopService` for per-frame updates
- Clean up Three.js resources in `DestroyRef.onDestroy()`
...
- `@hive-academy/angular-3d`: selector prefix `a3d-`
- `@hive-academy/angular-gsap`: selector prefix `agsp-`

## Module Index

- [angular-3d-demo](./apps/angular-3d-demo/CLAUDE.md) - Showcase demo application
- [angular-3d](./libs/angular-3d/CLAUDE.md) - Three.js Angular wrapper library
- [angular-gsap](./libs/angular-gsap/CLAUDE.md) - GSAP Angular wrapper library

## Library Imports

```typescript
// Three.js components and directives
import { Scene3dComponent, BoxComponent, Float3dDirective } from '@hive-academy/angular-3d';

// GSAP scroll animations
import { ScrollAnimationDirective, HijackedScrollDirective } from '@hive-academy/angular-gsap';
```
...
style(demo): format component templates
perf(angular-3d): optimize star field rendering
```
...
```bash
# Wrong type format
"Feature: Add search"           # Type not lowercase, wrong format

# Missing scope
"feat: add box component"       # Scope is required

# Invalid scope
"feat(three): add mesh helper"  # 'three' not in allowed scopes
"feat(lib): add new component"  # 'lib' not in allowed scopes

# Subject formatting
"feat(angular-3d): Add box."    # Uppercase + period
"feat(demo): Added new page"    # Past tense, should be imperative

# Header too long (max 100 chars)
"feat(angular-3d): add comprehensive box component with full material support and animation hooks"
```
...
```bash
# Scenario 1: Lint/Format error in current file
User chooses: Option 1 (Fix Issue)
Action: Run `npx nx format:write --all`, verify, retry commit

# Scenario 2: Type error in unrelated library (e.g., angular-gsap when working on demo)
User chooses: Option 2 (Bypass Hook)
Action: git commit --no-verify -m "feat(demo): add new scene component"
Document: Add note to tasks.md about bypassed hook

# Scenario 3: Pre-push build failure
User chooses: Option 3 (Stop & Report)
Action: Mark current task as blocked, run `npx nx affected:build` to diagnose
```
